//
//  SwiftLLMWrapper.swift
//
//
//  Created by Alec Dusheck on 7/16/24.
//  This is shamelessly taken from https://github.com/eastriverlee/LLM.swift/blob/main/Sources/LLM/LLM.swift
//

import Foundation
import llama

public typealias SwiftLLMToken = llama_token
public typealias SwiftLLMModel = OpaquePointer
public typealias SwiftLLMChat = (role: Role, content: String)

@globalActor public actor InferenceActor {
    static public let shared = InferenceActor()
}

open class SwiftLLMWrapper: ObservableObject {
    public var model: SwiftLLMModel
    public var history: [SwiftLLMChat]
    public var preprocess: (_ input: String, _ history: [SwiftLLMChat]) -> String = { input, _ in return input }
    ...
    private let newlineToken: SwiftLLMToken
    ...
    @InferenceActor
    private func predictNextToken() async -> SwiftLLMToken {
    ...
    private func process(_ token: SwiftLLMToken, to output: borrowing AsyncStream<String>.Continuation) -> Bool {
        struct saved {
    ...
    private func decode(_ token: SwiftLLMToken) -> String {
        return model.decode(token, with: &multibyteCharacter)
    }
    
    public func decode(_ tokens: [SwiftLLMToken]) -> String {
        return tokens.map({model.decodeOnly($0)}).joined()
    }
    
    @inlinable
    public func encode(_ text: borrowing String) -> [SwiftLLMToken] {
        model.encode(text)
    }
}

extension SwiftLLMModel {
    public var endToken: SwiftLLMToken { llama_token_eos(self) }
    public var newLineToken: SwiftLLMToken { llama_token_nl(self) }
    ...
    public func decodeOnly(_ token: SwiftLLMToken) -> String {
        var nothing: [CUnsignedChar] = []
        return decode(token, with: &nothing)
    }
    
    public func decode(_ token: SwiftLLMToken, with multibyteCharacter: inout [CUnsignedChar]) -> String {
        var bufferLength = 16
...
private class Context {
    ...
    init(_ model: SwiftLLMModel, _ params: llama_context_params) {
...
extension llama_batch {
    ...
    mutating func add(_ token: SwiftLLMToken, _ position: Int32, _ ids: [Int], _ logit: Bool) {
...
public struct Template {
    ...
    public var preprocess: (_ input: String, _ history: [SwiftLLMChat]) -> String {
